...
### TODO:
- You will want to update your test assertions to not rely on order when it is not explicitly set. One solution is to have two instances of folderTest with two different assertResult implementations - one which asserts order (for queries where this is specified) and one which does not (for queries where order is not specified).